...
class GameRoom(KBEngine.Base, GameObject, iRoomRules):
	...
	def broadcastMultiOperation(self, idx_list, aid_list, winType, tile_list=None):
		for i, p in enumerate(self.players_list):
			if p is not None:
	...
	def waitForOperation(self, idx, aid, tile, win_op = const.OP_BLAST_WIN): # 16 放炮胡 17明杠抢杠胡 18暗杠抢杠胡 19接杠抢杠胡
		#能不能胡(放炮)
		giveWin = False
		self.wait_idx_list = []
		self.wait_for_win_dict = {}
		...
		for i in range(len(player_list)):
			...
			if aid == const.OP_DISCARD or aid == const.OP_EXPOSED_KONG or aid == const.OP_CONCEALED_KONG:
				...
				self.wait_for_win_dict[wait_idx] = {"state": 0, "formIdx": idx, "win_op":win_op}
	...
	def processMutiWin(self, win_list, win_op, formIdx_list, tile):
		...
		for i in range(len(win_list)):
			...
			if win_op == const.OP_BLAST_WIN:
				score = 1
				if fromIdx == self.dealer_idx or win_idx_list[i] == self.dealer_idx: #有一方是庄家
