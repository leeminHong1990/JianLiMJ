...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	this.disconnect = function()
	{
		...
		{  
			if(KBEngine.app.socket != null)
			{
				...
				sock.close();
				// if(KBEngine.app.currserver != "loginapp"){
				// 	h1global.globalUIMgr.info_ui.show_by_info("连接关闭，请检查网络！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
				// }
			}
		}
		catch(e)
	...
	{  
		...
		KBEngine.Event.fire("onConnectStatus", false);
		h1global.globalUIMgr.info_ui.show_by_info("连接失败，请检查网络！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
	}
	...
	this.onerror_after_onopen = function(evt)
	{
		...
		h1global.globalUIMgr.info_ui.show_by_info("连接错误，请重新登录游戏！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
	}
	...
	{  
		...
		cc.log('connect close:' + KBEngine.app.currserver)
		KBEngine.Event.fire("onDisableConnect");
		if(KBEngine.app.currserver != "loginapp")
			KBEngine.app.reset();
		// h1global.globalUIMgr.hide_all_ui();
		// h1global.globalUIMgr.info_ui.show_by_info("连接已断开，请重新登录游戏！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
		// h1global.runScene(new LoginScene());
		if(cc.director.getRunningScene().className == "GameHallScene" || cc.director.getRunningScene().className == "GameRoomScene"){
			...
			if(info_json){
				...
				if(cc.sys.os == cc.sys.OS_ANDROID || cc.sys.os == cc.sys.OS_IOS){
					...
					KBEngine.app.login("wx_" + unionid, unionid, const_val.sClientDatas);
				} else {
					var unionid = info_dict["unionid"];
					KBEngine.app.login("wx_" + unionid, unionid, const_val.sClientDatas);
				}
	...
	{
		// cc.log("KBEngine.update:socket = ")
		// cc.log(KBEngine.app.socket)
		if(KBEngine.app.socket == null)
			return;
		...
		if((dateObject.getTime() - KBEngine.app.lastTickTime) / 1000 > 15)
		{
			...
			if(KBEngine.app.lastTickCBTime < KBEngine.app.lastTickTime)
			{
				...
				KBEngine.app.socket.close();
				h1global.globalUIMgr.info_ui.show_by_info("连接超时，请重新登录游戏！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
			}
	...
	*/
	...
	this.Client_onLoginFailed = function(args)
	{
		...
		h1global.globalUIMgr.info_ui.show_by_info("连接失败，请重新登录游戏！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
	}
	...
	this.Client_onKicked = function(failedcode)
	{
		...
		h1global.globalUIMgr.info_ui.show_by_info("与服务器断开连接，请稍后重新登录游戏！", cc.size(600, 400), function(){h1global.runScene(new LoginScene());})
	}
